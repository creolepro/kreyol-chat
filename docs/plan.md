...
**Probe result (Phase 0b, provisional — 2026-07-21, [report](../ml/reports/base_model_probe.md)):** ran the scorecard on 5 pretrained bases (bf16, Modal L40S). **`google/gemma-3-4b-pt` leads** — lowest authored-Kreyòl BPB (1.170) and highest eng→hat chrF2++ (43.1, ~1.7× the next), and the only model to recall a probe proverb. The Llama-3.2 control lands #2 on BPB (not the floor), and SmolLM3-3B is *worst* despite its French-coverage reputation (it did not transfer to this French-lexifier creole — a direct negative answer to that open question). **License caveat:** Gemma-3 is under the Gemma Terms, not Apache-2.0; if the Apache-2.0-clean bar above rules it out, the best clean base by BPB is **Qwen3-4B**. Final pick pends the blinded native-speaker naturalness review. These are the "before" numbers for the CPT before/after comparison.
...
- **Architecture constraint (decided now, saves pain later):** stay on nanochat's Llama-compatible (or a GPT-2-compatible) architecture so conversion to GGUF (`convert_hf_to_gguf.py`) and browser formats is turnkey. A custom architecture means hand-writing conversion code ([llama.cpp discussion](https://github.com/ggml-org/llama.cpp/discussions/19815)). *(Update 2026-07-22: the F smoke run proved the pinned nanochat speedrun arch is NOT convertible — decision recorded in [phase-1.md](./phase-1.md) Workstream G: train a standard Llama-architecture model inside the nanochat pipeline, depth chosen separately by validation BPB, with hard conversion gates (incl. stock-Ollama import and offline browser generation) before the flagship run.)*
