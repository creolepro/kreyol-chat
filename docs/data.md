...
| *— sweep-4 addendum (v0.2.1, executed 2026-07-24) —* | | [sweep4_ingest.md](../ml/reports/sweep4_ingest.md) | |
| **CMU Haitian Corpus** | ~443k tokens | **~433k** net-new (364 docs; **0** exact/near dup vs full v0.2 at chunk granularity) + **32,231** EN↔HT lexicon pairs (dictionary → lexicon, not LM text) | measured; registers news/health_translated |
| **Cric? Crac! (1901)** | ~25–50k | **~4.1k** (28 fable-run docs; +1 fable held out for the heritage exhibit) | measured; dLOC OCR (Creighton has no scan), page-level FR/HT split, pre-reform orthography |
| **Lessons in Haitian Creole (1921)** | ~10–20k | **0 — ABANDONED** (OCR: Kreyòl only as garbled conjugation-drill cells; 16 lines / 319 clean chars) | measured, not ingested (PD, recorded) |
| **Anthologie 1925** | <10k Kreyòl | **~1.0k** (8 Kreyòl-dense pages of 408; clean CC0 text layer) | measured; PyMuPDF, no OCR |

**Headline finding:** the *authored-unlock* thesis (VOA → triple the authored share to ~30%) did **not** survive measurement; VOA is register-unique but small. A *volume* surprise (fineweb-2 ~108M) cut the other way. Net v0.2 is a register-tagged corpus whose authored/register emphasis is driven by **train-mix weights**, not raw composition — see the v0.2 report for the achieved numbers. **Sweep-4 addendum (v0.2.1):** four PD/permissive sources fold in **+438k net-new tokens** and three new registers (news/health_translated, historical_literary) — register coverage, not scale (CMU is the bulk; the historical literary texts are tiny but unlike anything else in the corpus). The oldest, Cric? Crac! (1901), also yields the heritage exhibit fable.
...
- **Historical orthography** (sweep-4 PD texts, 1901/1921/1925): ✅ RESOLVED (v0.2.1 — [sweep4_ingest.md](../ml/reports/sweep4_ingest.md)). genre-tagged (`historical`/register `historical_literary`, `pre_reform` orthography), original spelling preserved EXACTLY (mix weight held at 1× — identifiable + down-weightable, not amplified). langid indeed failed on the spelling, so Kreyòl was isolated by a marker-based FR/HT pass then eyeballed. Outcomes: **Cric? Crac! 1901** (dLOC OCR — Creighton had only a catalog record; the 2011 L'Harmattan reprint left untouched) and **Anthologie 1925** (Manioc CC0, clean PDF text layer) ingested; **Lessons 1921 abandoned** (English manual; Kreyòl only as OCR-garbled drill cells — no extractable prose). One Cric? Crac! fable held out as the [heritage exhibit](../ml/reports/heritage_cric_crac_fable.md).
